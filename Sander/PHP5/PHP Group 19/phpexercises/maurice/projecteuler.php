...
//OP 2fibonacci
		...
		function palindrome(){
			...
			for($i = 100; $i < 1000; $i++){
				for($i2 = 100; $i2 < 1000; $i2++){
					...
					// compare
					if($lap == $pal){
						echo $i . " * " . $i2 . ": " .$lap . "<--->" . $pal . "<br>";
						
						$palres = array();
						array_push($palres, intval($lap));
					}
				}
			}
			$arrlength = count($palres);
			// echo $arrlength(); 
		}
		...
		function prime10001(){
			...
			for($i = 0; $i < 150000; gmp_nextprime($i)){
				...
				$primeArray[] = $i;
				echo "Number: " . count($primeArray) . ", " . $i . " added.<br>";
		...
		function digits(){
			echo "<br>";
			echo substr("appppeeeeeeeeeeeeeeeeeee", 1, 13);
			echo "<br>";
		...
		function primeMillions(){
			...
			for($i = 0; $i < 2000000; gmp_nextprime($i)){
				...
				$counter += $i;
				echo $i . " added, total is now: " . $counter . "<br>";
			}
		}
		// primeMillions();
		
		
		// OP11 - product werkt niet
		function multiGrid(){
			$grid = array(
